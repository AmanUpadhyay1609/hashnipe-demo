    ...
    >
      <div className="relative">
        ...
        <div className="absolute top-4 right-4 z-10 bg-dark-400/90 backdrop-blur-sm border border-dark-100 rounded-full p-1 flex items-center justify-center w-12 h-12">
          <div
            className={`text-sm font-bold flex items-center justify-center w-full h-full rounded-full ${score >= 70
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-2">
          <div>
            <Link to={`/tokens/${project.id}`} className="hover:text-primary-400 transition-colors">
              <h3 className="text-base font-medium text-white leading-tight">{project.virtual.name}</h3>
              <div className="text-xs text-primary-400 font-normal">${project.virtual.symbol}</div>
            </Link>
          </div>
        ...
        <p className="text-light-400 text-xs mb-4 line-clamp-2">
        ...
        <div className="space-y-4 mb-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-dark-400/60 rounded-lg p-2 text-center">
              <div className="text-xs font-medium text-white">{project.totalParticipants}</div>
              <div className="text-xs text-light-500">Users</div>
            </div>
            <div className="bg-dark-400/60 rounded-lg p-2 text-center">
              <div className="text-xs font-medium text-white">{project.totalVirtuals.toLocaleString()}</div>
              <div className="text-xs text-light-500">Virtuals</div>
            </div>
            <div className="bg-dark-400/60 rounded-lg p-2 text-center">
              <div className="text-xs font-medium text-white">{formatPercentage(subscriptionPercentage)}</div>
          ...
          <div>
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className="text-primary-400 font-normal">{formatPercentage(subscriptionPercentage)}</span>
            </div>
